...
def rng(a, b):
    # Округление до €500 схлопывает короткие строки: 250-450 с наценкой даёт
    # 500 и 500. «€500 – €500» читается как опечатка, а не как диапазон.
    if a == b:
        return money(a)
...
# ── 2. корпус ────────────────────────────────────────────────────────────
b2 = rows(SHELL) + total_row('The shell, complete', 'Корпус целиком', SH[0], SH[1])
...
b4 = rows(COFFEE_MID) + total_row('The coffee fit-out', 'Кофейная начинка', CM[0], CM[1])
